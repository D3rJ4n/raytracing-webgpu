...
export class ScreenProjection {
    ...
    /**
     * Berechne Shadow Bounds für eine Sphere
     * Diese Methode projiziert den Schatten einer Kugel auf den Ground
     */
    public calculateShadowBounds(
        ...
        // 1. Berechne Shadow-Ray Richtung
        const lightDir = this.normalize({
            x: lightPosition.x - spherePosition.x,
            y: lightPosition.y - spherePosition.y,
            z: lightPosition.z - spherePosition.z
        });

        // 2. Finde Schnittpunkt des Shadow Rays mit Ground
        // Ray: P = spherePosition + t * (-lightDir)
        // Ground: y = groundY
        // Löse für t: spherePosition.y + t * (-lightDir.y) = groundY

        if (Math.abs(lightDir.y) < 0.001) {
            // Licht ist parallel zum Ground → kein definierter Schatten
            return this.emptyBounds();
        }

        const t = (spherePosition.y - groundY) / lightDir.y;

        if (t <= 0) {
            // Schatten ist hinter der Kugel oder Kugel unter Ground
            return this.emptyBounds();
        }

        // 3. Schatten-Center auf Ground
        const shadowCenter = {
            x: spherePosition.x - lightDir.x * t,
            y: groundY,
            z: spherePosition.z - lightDir.z * t
        };

        // 4. Schatten-Radius approximieren
        // Der Schatten wird größer je weiter weg von der Lichtquelle
        const distanceToLight = Math.sqrt(
            Math.pow(spherePosition.x - lightPosition.x, 2) +
            Math.pow(spherePosition.y - lightPosition.y, 2) +
            Math.pow(spherePosition.z - lightPosition.z, 2)
        );

        const distanceToGround = Math.abs(spherePosition.y - groundY);

        // Shadow radius = sphere radius + penumbra (grobe Approximation)
        const shadowRadiusApprox = sphereRadius * (1.0 + distanceToGround / Math.max(distanceToLight, 1.0)) * 1.5;

        // 5. Projiziere Shadow-Region auf Screen
        const shadowBounds = this.sphereToScreenBounds(shadowCenter, shadowRadiusApprox);
